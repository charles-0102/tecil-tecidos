...
#
# Este é o "swipe" do local: stagea tudo, commita e pusha pra main. Como o
# Vercel deste projeto builda a produção a cada push em `origin/main`, esse
# push É o gatilho de deploy — não roda `vercel` aqui.
#
# Fluxo:
#   1. valida branch + repo
#   2. checa arquivos sensíveis ANTES de stagear
#   3. stagea tudo (inclui untracked) e commita (auto-tag TCL-JM-NN)
#   4. detecta remotes; se >1, pusha pra todos (ou --to <nome>)
#   5. fetch + rebase em cima do remote primário (histórico linear)
...
#   scripts/sync-main.sh --dry-run                # mostra o que faria, sem tocar em nada
...
if [[ -n "$pending_files" ]]; then
  ...
  if [[ -z "$COMMIT_MSG" ]]; then
    # próximo número TCL-JM-NN (procura em git log; tolerante a "nenhum match")
    last_n="$({ git log --all --pretty=%s 2>/dev/null || true; } \
      | { grep -oE '^TCL-JM-[0-9]+' || true; } \
      | { grep -oE '[0-9]+$' || true; } \
      | sort -n | tail -1)"
    next_n=$(( 10#${last_n:-0} + 1 ))
    tag="$(printf 'TCL-JM-%02d' "$next_n")"
    ...
    areas="$({ git diff "${diff_src[@]}" --name-only 2>/dev/null || true; } | awk -F/ '
      $1=="app"                     { print "app"; next }
      $1=="components" && NF>=2      { print "components/"$2; next }
      $1=="components"               { print "components"; next }
      $1=="lib" && NF>=3             { print "lib/"$2; next }
      $1=="lib"                      { print "lib"; next }
      $1=="hooks"                    { print "hooks"; next }
      $1=="types"                    { print "types"; next }
      $1=="scripts"                  { print "scripts"; next }
      $1=="public"                   { print "public"; next }
      $1=="assets"                   { print "assets"; next }
                                     { print $1 }
...
if [[ $DRY_RUN -eq 0 ]]; then
  ...
  dim "▸ push em $MAIN_BRANCH disparou o build de produção no Vercel."
fi
